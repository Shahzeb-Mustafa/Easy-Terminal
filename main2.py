...
class NaturalLanguageTerminal:
    ...
    def detect_command_type(self, command):
        ...
        """
        # Common bash commands regex pattern
        bash_pattern = r'^(cd|ls|mkdir|rm|cp|mv|grep|cat|echo|pwd|sudo|apt|git|touch|chmod|chown|man|find|ps|kill|top|df|zip|unzip|tar|ssh|scp|ping|ifconfig|ip|curl|wget|history)( .*)?$'
        
        # Check if it's a recognized bash command
        if re.match(bash_pattern, command) or '|' in command or ';' in command or '>' in command:
            return 'bash'
        ...
        if '/' in command or '~' in command or ('"' in command and '"' in command[command.index('"')+1:]):
            return 'bash'
            
        # Check for common natural language indicators
        natural_indicators = ['show', 'list', 'display', 'find', 'get', 'what', 'how', 'create', 'make', 'write']
        
        for indicator in natural_indicators:
            if command.lower().startswith(indicator):
                return 'natural'
    ...
    def process_command(self, command):
        ...
        if command_type == 'bash':
            ...
            try:
                ...
                self.append_output(f"Error translating command: {str(e)}\n")
